...
setwd("C:/Users/jingzhi/Desktop/Site-basedPredictionModel/data/training set/NH/H1N1")

###################### current consensus strain in time T #######################
##The function GetCurrentStrain allows to calculate current consensus sequence in time T as vaccine strain in T+1

load("NHH1_HA_sequence.Rdata")
load("NHH1_NA_sequence.Rdata")

GetCurrentStrain <- function(input_csv_seq, predict_season){
...
## for H1N1, predict_season=2012-2022
## for H3N2, predict_season=2002-2022

ha_predicted_conseq <- GetCurrentStrain(input_csv_seq = ha_allseq, predict_season = c(2012:2022))
na_predicted_conseq <- GetCurrentStrain(input_csv_seq = na_allseq, predict_season = c(2012:2022))

## output

write.csv(ha_predicted_conseq,file="NHH1_HA_current consensus.csv")
write.csv(na_predicted_conseq,file="NHH1_NA_current consensus.csv")


###################### answer strain in time T+1 #######################
## The function GetAnswerCon allows to calculate consensus sequence in time T+1
...
ha_answer_conseq <- GetAnswerCon(input_csv_seq = ha_allseq, predict_season = c(2012:2022))
na_answer_conseq <- GetAnswerCon(input_csv_seq = na_allseq, predict_season = c(2012:2022))

## output

write.csv(ha_answer_conseq,file="NHH1_HA_answer consensus.csv")
write.csv(na_answer_conseq,file="NHH1_NA_answer consensus.csv")
...
GetAnswerWT<- function(input_ha_seq, input_na_seq, ha_pre_seq, na_pre_seq, pre_year){
  ...
  alpha <- 0.5 # equal woght for HA and NA distance
...
answer_wildtype_result <- GetAnswerWT(input_ha_seq = ha_allseq, input_na_seq = na_allseq, 
                                      ha_pre_seq <- read.csv("NHH1_HA_answer consensus.csv"),
                                      na_pre_seq <- read.csv("NHH1_NA_answer consensus.csv"),
                                      pre_year = c(2012:2022))

## output

write.csv(answer_wildtype_result$answer_ha,file="NHH1_HA_answer strain.csv",row.names = F)
write.csv(answer_wildtype_result$answer_na,file="NHH1_NA_answer strain.csv",row.names = F)
